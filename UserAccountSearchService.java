...
//        paramMap.put("state", "15");
    ...
    public Object searchFinanceInvest(PageVo pageVo){
        ...
        for(int i =0;i<list.size();i++){
            ...
            if(!StringUtils.isEmpty(map.get("lock_issue"))){
                ...
                //新手荷包加息按照利息计算
                if(!StringUtils.isEmpty(map.get("add_rate"))){
                    lockInterest = this.calculateAddRateInterest(dealMoney,rate,lockIssue);
                }else{
                ...
                if(!StringUtils.isEmpty(map.get("add_rate"))){
                    ...
                    interest=new MoneyCalculator(interest).add(lockInterest).toResult();
                }else{
     ...
     * @return
     */
    public BigDecimal calculateAddRateInterest(String dealMoney,String rate,String issue){
        BigDecimal interest = BigDecimal.ZERO;
        //加入金额*荷包利率/12）*荷包总期数
        interest = new MoneyCalculator(dealMoney).multiply(new BigDecimal(rate)).divide(new BigDecimal(100)).divide(new BigDecimal(12)).
                multiply(new BigDecimal(issue)).toResult(16);
        return interest;
    }
    ...
    public Object searchFinanceJoin(PageVo pageVo){
        ...
        for(int i =0;i<list.size();i++){
            ...
            if(!StringUtils.isEmpty(map.get("lock_issue"))){
                ...
                map.put("lockInterest",new MoneyCalculator(lockInterest).toResult());
